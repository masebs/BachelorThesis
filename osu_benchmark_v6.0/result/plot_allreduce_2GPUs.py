...
def readLists(dataset, function, machine):
    logfile = dataset+'/log-'+function+'_'+machine+'_2GPUs.log'
...
plt.ylabel('Time [µs]')
plt.title(function1 + ' vs ' + function2 + ' on ' + machine + ' (' + dataset + '), 2 GPUs')
plt.legend()
plt.grid()
logfile  = 'log-'+function1+'_'+machine
fig1.savefig(dataset+'/'+logfile+'_lineplot_2GPUs.png', dpi=300, bbox_inches='tight')
...
plt.grid()
logfile  = 'log-'+function1+'_'+machine
fig2.savefig(dataset+'/'+logfile+'_lineplot-log_2GPUs.png', dpi=300, bbox_inches='tight')
...
if len(comp1) > 0:
    ...
    plt.xlabel('log2(message size [Bytes])')
    plt.ylabel('Time [µs]')
    plt.title(function1 + ' on ' + machine + ' (' + dataset + '), 2 GPUs')
    plt.legend()
    plt.grid()
    logfile  = 'log-'+function1+'_'+machine
    fig3.savefig(dataset+'/'+logfile+'_barplot-detail_2GPUs.png', dpi=300, bbox_inches='tight')
    ...
    plt.grid()
    logfile  = 'log-'+function1+'_'+machine
    fig4.savefig(dataset+'/'+logfile+'_barplot_2GPUs.png', dpi=300, bbox_inches='tight')
