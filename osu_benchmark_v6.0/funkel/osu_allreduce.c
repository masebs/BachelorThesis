...
//   fileptr = fopen("/home/00/mschneider/ba/ndzip/test/msg_sweep3d.fp64", "rb");
//   fileptr = fopen("/home/00/mschneider/ba/ndzip/test/num_brain.fp64", "rb");
...
int main(int argc, char *argv[])
{
    ...
    cudaErrorCheck( cudaMalloc(&d_compReceive_buffer_mem, memory_buffer) );
    ...
    d_compressed_buffer = (double*)d_compressed_buffer_mem;
    d_compReceive_buffer = (double*)d_compReceive_buffer_mem;
    ...
    cudaErrorCheck( cudaMemset(d_compReceive_buffer, 0, memory_buffer) );
...
//         fill_buffer_constant(send_buffer, size);
        int firstStageCompressedLength = 0;
        ...
        for (i = 0; i < options.iterations + options.skip; i++) {
            ...
            for (int mask = 1; mask < numprocs; mask <<= 1) {
                ...
                compressed_length = NDZIP_API_compressBuffer(d_send_buffer, size, 
                                    (long unsigned int *)d_compressed_buffer);
                cudaErrorCheck( cudaDeviceSynchronize() );
                t_stop_comp = MPI_Wtime();
                ...
                MPI_CHECK( MPI_Sendrecv(d_compressed_buffer, compressed_length, 
                                        ...
                                        MPI_DOUBLE, partner, 0, MPI_COMM_WORLD,
                                        MPI_STATUS_IGNORE) );
                t_stop_comm = MPI_Wtime();
                
                // Decompress received data and reduce it
                t_start_decomp = MPI_Wtime();
                double *buffer_ptr = 
                    NDZIP_API_decompressBuffer((long unsigned int *)d_compReceive_buffer,
                                            d_receive_buffer, size);  
                cudaErrorCheck( cudaDeviceSynchronize() );
                t_stop_decomp = MPI_Wtime();
                ...
                if (i >= options.skip) {
                    timer_comp += t_stop_comp - t_start_comp;
                    timer_comm += t_stop_comm - t_start_comm;
                    timer_decomp += t_stop_decomp - t_start_decomp;
                    timer_reduce += t_stop_reduce - t_start_reduce;
                }
                
                if (i == 0 && mask == 1) {
                    // only in the first butterfly stage of the first iteration for this length
                    // (all later results should be equal): get avg compressed lengths
                    // in order to be able to judge the compressibility of the data; this
                    // will later be collected from all processes via reduce
                    firstStageCompressedLength = (compressed_length + compressed_length_partner)
                                                 / 2 * sizeof(double);
                }
            }
        ...
        timer_comp = (double)(timer_comp * 1e6) / options.iterations;
        timer_comm = (double)(timer_comm * 1e6) / options.iterations;
        timer_decomp = (double)(timer_decomp * 1e6) / options.iterations;
        ...
        MPI_CHECK(MPI_Reduce(&timer_comp, &avg_comp_time, 1, MPI_DOUBLE, MPI_SUM, 0,
                MPI_COMM_WORLD));
        avg_comp_time /= numprocs;
        MPI_CHECK(MPI_Reduce(&timer_comm, &avg_comm_time, 1, MPI_DOUBLE, MPI_SUM, 0,
                MPI_COMM_WORLD));
        avg_comm_time /= numprocs;
        MPI_CHECK(MPI_Reduce(&timer_decomp, &avg_decomp_time, 1, MPI_DOUBLE, MPI_SUM, 0,
                MPI_COMM_WORLD));
        avg_decomp_time /= numprocs;
        MPI_CHECK(MPI_Reduce(&timer_reduce, &avg_reduce_time, 1, MPI_DOUBLE, MPI_SUM, 0,
                MPI_COMM_WORLD));
        avg_reduce_time /= numprocs;
        MPI_CHECK(MPI_Reduce(&firstStageCompressedLength, &avgCompressedLength, 1, 
                             MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD));
        avgCompressedLength /= numprocs;
        ...
        if (options.validate) {
            print_stats_validate_detailed(rank, size, avgCompressedLength, latency, min_time,
                    max_time, errors, avg_comp_time, avg_comm_time, avg_decomp_time,
                    avg_reduce_time);
        } else {
            print_stats_detailed(rank, size, avgCompressedLength, latency, min_time,
                    max_time, avg_comp_time, avg_comm_time, avg_decomp_time, avg_reduce_time);
        }
